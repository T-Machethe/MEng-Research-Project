...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--max_files", type=int, default=9999,
        help="Max files to scan (use a small number like 100 for a quick test run)"
    )
    # Allow loading from cache to skip rescanning
    parser.add_argument(
        "--from_cache", action="store_true",
        help="Load metadata from eda_metadata_cache.csv instead of rescanning"
    )
